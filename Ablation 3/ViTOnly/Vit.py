...
vit_ckpt_dir       = "/home/qitam/sdb2/home/qiteam_project/ahmad/BrainTumor/pretrain/Vit"
batch_size         = 128
...
image_size         = 224
num_folds          = 5
output_dir         = "/home/qitam/sdb2/home/qiteam_project/ahmad/BrainTumor/Paper code/ViTOnly"
...
class ViTClassifier(nn.Module):
    def __init__(self, vit_dir, num_cls):
        super().__init__()
        # load config + weights from local directory
        config = ViTConfig.from_pretrained(vit_dir)
        self.vit = ViTModel.from_pretrained(vit_dir, config=config)
        # classification head
...
for fold, (trainval_idx, test_idx) in enumerate(
        skf.split(np.zeros(len(targets)), targets), 1):
    print(f"\n=== Fold {fold}/{num_folds} ===")
    ...
    # instantiate ViT classifier
    model = ViTClassifier(vit_ckpt_dir, num_classes).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(
                output_dir, f"best_vit_fold{fold}.pth"))
            early_stop_ctr = 0
        else:
            early_stop_ctr += 1
...
# ─── Save JSONs ────────────────────────────────────────────────────────────────
metrics_path = os.path.join(output_dir, f"vit_metrics_{timestamp}.json")
with open(metrics_path, 'w') as fp:
    json.dump(fold_metrics, fp, indent=4)

accuracies = {f"fold_{m['fold']}": m["test_accuracy"] for m in fold_metrics}
acc_path    = os.path.join(output_dir, f"vit_accuracies_{timestamp}.json")
