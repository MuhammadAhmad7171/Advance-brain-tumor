...
# ─── Fusion Modules ─────────────────────────────────────────────────────────────
class NaiveFusionConcat(nn.Module):
    ...
    def forward(self, r, v):
        return self.fc(torch.cat([r, v], dim=1))

class HybridNaiveFusionModel(nn.Module):
    def __init__(self, resnet, vit, fusion_module, aligned_dim):
        super().__init__()
        # remove final FC from ResNet
    ...
    def forward(self, x):
        r = self.resnet_proj(self.resnet(x))
        v = self.vit_proj(self.vit(x).last_hidden_state[:,0,:])
        return self.fusion(r, v)
...
for fold, (trainval_idx, test_idx) in enumerate(skf.split(np.zeros(len(targets)), targets), 1):
    ...
    fusion_layer = NaiveFusionConcat(aligned_dim, num_classes)
    model = HybridNaiveFusionModel(resnet, vit, fusion_layer, aligned_dim).to(device)
